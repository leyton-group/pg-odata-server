import { Edm, odata } from "ts-odata-v4-server";
...
export class Product {
  ...
  async invertDiscontinued(@odata.result result: Product) {
    ...
    await db.query(
      `UPDATE "Products" SET "Discontinued" = $1 WHERE "id" = $2`,
      [!result.Discontinued, result.id]
    );
  }

  @Edm.Action
  async setDiscontinued(
    @odata.result result: Product,
